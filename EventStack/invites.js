...
client.on("ready", async() => {
    ...
    client.guilds.cache.forEach(async guild => {
        ...
        try {  
            ...
            invites.set(guild.id, new Map(firstInvites.map(invite => [invite.code, invite.users])))
...
client.on("guildMemberAdd", async member => {
    ...
    guild.invites.fetch().then(async newInvites => {
        ...
        const Embed = new EmbedBuilder()
            ...
            .setTitle("İnvite Logged")
            .setTimestamp()

        if (!invite) return Channel.send({
            embeds: [
                Embed.setDescription(`${user.tag} Katıldı ama, Kimin Davet Ettiğini Bulamadım`)
            ]
        })
        ...
        djsClient.grabProfile(invite.inviter.id).then(async inviter => {
            ...
            return Channel.send({
                embeds: [
                    Embed.setDescription(`${user.tag} davet kodunu kullanarak katıldı ${invite.code} Davet Eden Kişi ${Inviter}. Toplam ${invite.uses} kez Sunucumuza Üye Davet Etti!`)
                ]
            })
        }).catch(err => { })
    }).catch(err => {
